...
const { generateChiefBrief, generateWisdomInsights, generateEmailBriefs } = require('./src/services/briefing-ai');
...
app.get('/api/briefing', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      const startOfTodayUtc = new Date(require('./src/util/date').naiveToUtcIso(`${todayLocalDateStr}T00:00:00`, tz));
  ...
  require('./src/services/brief-audio').prewarmDaily(response).catch((err) => console.error('[brief audio prewarm] failed:', err.message));
