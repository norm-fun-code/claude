...
async function todaySpendWithBaseline() {
  try {
    ...
    const to = new Date(from.getTime() + 24 * 60 * 60 * 1000); // exclusive upper bound — tomorrow 00:00Z
    ...
    const [todayRows, baselineRows] = await Promise.all([
      metricsStore.dailyAggregate({ domain: 'wealth', metric: 'spending_discretionary', from, to, agg: 'sum', excludeSource: 'seed' }),
