...
async function ask(question, { history = [], k = 14, voice = false } = {}) {
  ...
  const [
    ...
    // Gated to personal questions for the same reason recovery context is: it
    // is a real (if cached) scan of the spine, and an impersonal question has
    // no use for it. Served from the same memo the /api/precedent route fills,
    // so a question asked right after opening the app is a cache hit rather
    // than a second scan.
...
module.exports = {
  ask, buildPrompt, isPersonalQuestion, isFinancialQuestion, personalSnapshot, renderSnapshot, recoveryContext, recoveryContextFrom, wealthContext, precedentContext,
