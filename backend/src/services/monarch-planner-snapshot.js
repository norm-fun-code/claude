...
function makeSnapshot(accounts, { asOf = new Date().toISOString(), source = 'normos-api' } = {}) {
  ...
  for (const account of accounts) {
    ...
    if (!name || !Number.isFinite(balance) || ids.has(id)) return null;
    ids.add(id);
    clean.push({ id, displayName: name, currentBalance: balance });
  }
  return { version: 1, accounts: clean, asOf: new Date(asOf).toISOString(), source };
}
