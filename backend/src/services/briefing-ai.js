...
function buildPrompt(emailData, notionText, quote, currentDay, workoutPlan, calendarEvents, wellbeingContext = '', annotationsContext = '', recoveryContext = '', experimentsContext = '', selfModel = '', leverageContext = '', workBusyBlocks = [], strengthContext = '', spendingContext = '', streakContext = '') {
...

{
  "chiefBrief": {
    ...
    "move": "THE MOVE (1-2 sentences). The single most consequential thing that CHANGED and why it matters. HARD RULE: never mention net worth or any net-worth figure or percentage, and never tie a balance to their work — it's noise in a daily brief. Wealth belongs here ONLY as a spending or cashflow insight, and ONLY when something is genuinely notable (a spend spike, an unusual category, savings rate or budget pace moved meaningfully). If spending is unremarkable, do NOT force a finance line — draw THE MOVE from a real changed number in habits, recovery, or sleep instead. Name the before→after and the implication."
  },
...
async function generateBriefing(emailData, notionText, quote, currentDay, workoutPlan, calendarEvents, wellbeingContext = '', annotationsContext = '', recoveryContext = '', experimentsContext = '', selfModel = '', leverageContext = '', workBusyBlocks = [], strengthContext = '', spendingContext = '', streakContext = '') {
  ...
  const prompt = buildPrompt(filteredEmails, notionText, quote, currentDay, workoutPlan, calendarEvents, wellbeingContext, annotationsContext, recoveryContext, experimentsContext, selfModel, leverageContext, workBusyBlocks, strengthContext, spendingContext, streakContext);
