...
/**
 * Smoke test: mint a token, then make a minimal Anthropic call with Monarch's
 * MCP attached, asking Claude to list the available Monarch tools. Returns the
 * raw content blocks so a debug endpoint can show the token status, which tools
 * surfaced (mcp_tool_use), and their results (mcp_tool_result).
 */
async function probe() {
  ...
  try {
    ...
    return { tokenOk: false, error: err.response?.data || err.message };
  }
  ...
  return {
    tokenOk: true,
    accessTokenExpiresInSec: cachedAccess ? Math.round((cachedAccess.expiresAt - Date.now()) / 1000) : null,
    stopReason: data.stop_reason,
    toolsCalled: blocks.filter((b) => b.type === 'mcp_tool_use').map((b) => ({ name: b.name, server: b.server_name, input: b.input })),
    toolResults: blocks
      .filter((b) => b.type === 'mcp_tool_result')
      .map((b) => ({ isError: b.is_error, content: b.content })),
    text: blocks.filter((b) => b.type === 'text').map((b) => b.text).join(''),
  };
}

module.exports = { isConfigured, getAccessToken, answerWithMonarch, probe, MCP_URL };
