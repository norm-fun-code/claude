...
const ADVISOR_TOOLS = [
  {
    ...
    description: 'Change one named annual living-expense category in one plan year. Use operation=set for "set Misc to $20K", increase for "add/increase Misc by $20K", and decrease for "reduce Misc by $20K". This places the resulting value inside Living for that year only; it never changes the baseline or later years.',
    input_schema: {
      type: 'object',
      properties: {
      ...
      required: ['year','category','operation','amount','reason'],
    },
  },
  {
    name: 'set_param',
...
// The overrides, the scrub and the household itself live in public/demo-data.js, shared with
// the browser's Demo Mode so the two cannot drift into different households — or into one that
// still carries a real figure the other had replaced.
...
function buildDemoPage() {
  try {
    ...
    const demo = Demo.demoDefaults(real);
...
Tools available:
• set_expense_category(year, category, operation, amount, reason) — change a named category for one year only. "Set/to" means operation=set. "Add/increase/by" means operation=increase. "Reduce/decrease/by" means operation=decrease. It rolls into Living, never the one-off column, and never changes later years.
...
• Named-category changes are NOT one-offs. If he names Groceries, Dining, Shopping, Vacations, Auto, Insurance, Misc, Entertainment, Charity, Medical, Transit, or Utilities, call set_expense_category. "Set 2027 Misc to $13,125" sets the total to $13,125. "Add $20K to 2027 Misc" increases the model's existing 2027 Misc total by $20K. Both affect 2027 only; neither changes baseMisc nor any later year.
      ...
      if (msg.stop_reason === 'tool_use') {
        ...
        for (const block of msg.content) {
          ...
          if (block.name === 'set_expense_category') {
            const {year,category,operation,amount,reason}=block.input;
            ...
            if(!Number.isInteger(Number(year))||idx<0||idx>10){
            ...
            }else if(!['set','increase','decrease'].includes(operation)){
              result={error:'Operation must be set, increase, or decrease.'};
            }else if(!Number.isFinite(Number(amount))||Number(amount)<0){
              result={error:'Category amount must be a non-negative dollar amount.'};
            }else{
              ...
              const next=operation==='increase'?old+dollars:operation==='decrease'?Math.max(0,old-dollars):dollars;
              aiParams[key]=next;
              proposedChanges.push({key,oldValue:old,value:next,reason,expenseCategory:category,expenseYear:Number(year),expenseOperation:operation,expenseAmount:dollars});
              send({tool_call:{name:'set_expense_category',key,oldValue:old,value:next,reason,expenseCategory:category,expenseYear:Number(year),expenseOperation:operation,expenseAmount:dollars}});
              result={ok:true,year:Number(year),category,operation,amount:dollars,value:next,previousValue:old};
            }
