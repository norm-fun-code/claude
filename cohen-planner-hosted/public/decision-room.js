...
function renderDecisionRoom(R){
  ...
  document.getElementById('chartArea').innerHTML=`<div class="decision-room">
    <section class="dr-intro">
      ...
      <div class="dr-now"><div class="dr-kicker">${todayYear} PLANNED MONTHLY MARGIN</div><strong>${decisionMoney(today.flow/12)}</strong><span>All after-tax pay less modeled expenses${today.gap>0?` · ${fmt(today.gap)}/yr of it comes from selling vest`:''}</span><div class="dr-source">${decisionEsc(source)}</div></div>
    </section>
    <div class="dr-metrics">
      ...
      <article><span>Tightest cash-flow year · ${summary.tightest.yr}</span><strong class="${summary.tightest.flow<0?'dr-negative':''}">${decisionMoney(summary.tightest.flow/12)}<small>/mo</small></strong><p>Net flow ÷ 12 — all income, all spending. Home down payment is separate.</p><button class="dr-text-button" onclick="decisionSelectYear(${summary.tightest.yr})">Explore this year →</button></article>
    </div>
...
function decisionRenderChart(base,preview,changed){
  ...
  // Net worth only — the same series the Projection tab's chart and the scenario cases
  // plot. Adding the 401k here made this chart sit above every other view of the same plan.
  const series=rows=>rows.map(r=>decisionDollars(r.nw,r.yr));
