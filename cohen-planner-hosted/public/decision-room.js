...
function renderDecisionRoom(R){
  ...
  document.getElementById('chartArea').innerHTML=`<div class="decision-room">
    ...
    <div class="dr-metrics">
      <article><span>Before closing · ${P.homePurchaseYear}</span><strong>${summary.closingBuffer===null?'Outside horizon':fmt(summary.closingBuffer)}</strong><p>${summary.closingBuffer===null?'Choose a purchase year within the plan.':`Prior balance less ${fmt(summary.down)} down payment. Before closing costs and sale taxes.`}</p></article>
    ...
    <details class="dr-method"><summary>What these numbers include</summary><p>All previews use your existing annual projection and tax model. Total wealth includes liquid assets, home equity and the modeled 401k. Liquid assets include investments and are not the same as cash. Annual margin excludes the home down payment. The closing buffer uses the previous year-end balance (or starting liquid in the first year), less the down payment, and excludes closing costs and taxes on sales to fund that payment. The existing model continues employment income and retirement contributions through the end of the plan; this is not a retirement drawdown simulation. Returns are assumptions, not guaranteed outcomes. Changing home price holds maintenance and other expenses fixed unless you edit them in Settings. Changing childcare applies the monthly rate to each eligible child.</p></details>
  </div>`;
