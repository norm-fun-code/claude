...
function renderDecisionRoom(R){
  ...
  document.getElementById('chartArea').innerHTML=`<div class="decision-room">
    <section class="dr-intro">
      ...
      <div class="dr-now"><div class="dr-kicker">${todayYear} PLANNED MONTHLY MARGIN</div><strong>${decisionMoney(today.flowMonthly)}</strong><span>All after-tax pay less modeled expenses${today.gap>0?` · ${fmt(today.gap)}/yr of it comes from selling vest`:''}</span><div class="dr-source">${decisionEsc(source)}</div></div>
    </section>
